...
execute as @e[tag=junglemonster,distance=..10,scores={hades.downward=1..}] run function utility:damage {target: "@s", amount: 5, type: "wither", attacker: "@a[tag=self,limit=1]"}
execute as @e[tag=junglemonster,distance=..10,scores={hades.downward=1..}] run effect give @s wither 5 1
execute as @e[tag=junglemonster,distance=..10,scores={hades.downward=1..}] run scoreboard players set @s hades.downward 0
tag @s remove self
