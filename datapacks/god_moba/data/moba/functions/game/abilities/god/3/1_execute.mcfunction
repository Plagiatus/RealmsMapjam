...
execute positioned ~-.5 ~-.5 ~-.5 as @e[tag=junglemonster,dx=0,dy=0,dz=0] run function utility:damage {target: "@s", amount: 5, type: "wither", attacker: "@a[tag=self,limit=1]"}
execute positioned ~-.5 ~-.5 ~-.5 as @e[tag=junglemonster,dx=0,dy=0,dz=0] run scoreboard players set @s hades.downward 160