...
execute if score @s tmp matches 0 store result score @s tmp if entity @e[tag=junglemonster,distance=..1.5]
...
execute as @e[tag=junglemonster,distance=..1.5] run function utility:damage {target: "@s", amount: 6, type: "minecraft:falling_stalactite", attacker: "@a[scores={god=1},team=t2,limit=1,sort=nearest]"}
